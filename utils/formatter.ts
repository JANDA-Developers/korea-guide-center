import { toNumber } from "@janda-com/front";
...
export function toOps<T, C extends keyof T>(
    op: Array<T>,
    id: C,
    label: C
): IselectedOption[] {
...
export const withWon = (price: number | string = 0) => {
    return `₩ ${autoComma(price)}`;
};

export const moneyShortyfy = (price: number | string) => {
    let Price = price as number;
    if (typeof price === "string") Price = toNumber(price);

    const over = Math.floor(Price / 10000);
    const under = (Price % 10000) / 10000;
    return over + under;
};

const kone = (money: number) => {
    return Math.floor(money / 1000);
};
const koneWithHund = (money: number) => {
    const under = money % 1000;
    const hundred = Math.floor(under / 100);
    return kone(money) + "." + hundred + "k";
};

export const mobileShortyFyWithOne = (price: number | string = 0) => {
    if (typeof window === "undefined") return withWon(price);
    if (window.outerWidth < 500) return moneyShortyfy(price);
    else return koneWithHund(toNumber(price));
};
