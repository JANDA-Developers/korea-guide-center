...
type Props = {
    ...
    animateIn?: TanimationKey;
    afterAnimatedIn?: (visi: TVisi) => void;
    animateOut?: string;
    delay?: number;
...
export const AnimationOnScroll = ({
    ...
    const getScrollPos = useCallback(() => {
        ...
        return scrollableParentRef.current?.scrollTop || 0;
    }, [scrollableParentRef]);
    ...
    const getScrollableParentHeight = useCallback(() => {
        ...
        return scrollableParentRef.current?.clientHeight || 0;
    }, [scrollableParentRef]);
    ...
    const getVisibility = useCallback(() => {
        if (!scrollableParentRef) return { inViewport: false, onScreen: false };
        const elementTop =
            getElementTop(node.current) -
            getElementTop(scrollableParentRef.current);
        const elementBottom = elementTop + (node.current?.clientHeight || 0);
    ...
    useEffect(() => {
        if (!serverSide) {
            ...
            console.log({ scrollableParentRef });
    ...
    return (
        <div
            ref={node}
            className={
                classNameProps ? `${classNameProps} ${classes}` : classes
            }
